...
const SalesRepOrderHistory = () => {
	...
	// Filter orders to show only sales rep orders (orders with offline payment methods)
	const salesRepOrders = useMemo(() => {
		return allOrders.filter((order) => {
			// Check if order was created by sales rep (based on payment method or payment details)
	...
	const orderStats = useMemo(() => {
		...
		const todayOrders = salesRepOrders.filter(o => 
		...
		return {
			total: salesRepOrders.length,
			today: todayOrders.length,
			todayRevenue,
			pending: salesRepOrders.filter((o) => o.status === "pending").length,
			confirmed: salesRepOrders.filter((o) => o.status === "confirmed").length,
			shipped: salesRepOrders.filter((o) => o.status === "shipped").length,
			delivered: salesRepOrders.filter((o) => o.status === "delivered").length,
			cancelled: salesRepOrders.filter((o) => o.status === "cancelled").length,
		};
	}, [salesRepOrders]);
	...
	const filteredOrders = useMemo(() => {
		return salesRepOrders.filter((order) => {
			const matchesSearch =
	...
	}, [salesRepOrders, searchTerm, statusFilter, paymentFilter, dateRange]);
	...
	const handleExport = () => {
		try {
			// Create CSV content
			const csvContent = [
				["Order Number", "Customer", "Phone", "Total", "Payment Method", "Status", "Date"],
			...
			<div className="space-y-6">
				<div className="flex justify-between items-center">
					<div>
						...
						<p className="text-gray-600">View and manage your created orders</p>
					</div>
				</div>
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
				...
				<div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
					<div className="flex items-center">
						...
						<div className="ml-4">
							<h3 className="text-lg font-semibold text-gray-900">{orderStats.today}</h3>
				...
				<div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
					<div className="flex items-center">
						...
						<div className="ml-4">
							<h3 className="text-lg font-semibold text-gray-900">{formatPrice(orderStats.todayRevenue)}</h3>
				...
				<div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
					<div className="flex items-center">
						...
						<div className="ml-4">
							<h3 className="text-lg font-semibold text-gray-900">{orderStats.confirmed}</h3>
						...
						<tbody className="bg-white divide-y divide-gray-200">
							{paginatedOrders.map((order) => (
								<tr key={order.id} className="hover:bg-gray-50">
									...
									<td className="px-6 py-4 whitespace-nowrap">
										<span
											className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium ${getPaymentMethodColor(
												order.paymentMethod,
											)}`}
										>
						...
						<div className="p-6 space-y-6">
							...
							<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
								...
								<div className="bg-gray-50 p-4 rounded-lg">
									...
									<span
										className={`inline-flex px-3 py-1 rounded-full text-sm font-medium ${getPaymentMethodColor(
							...
							{/* Payment Details */}
							{selectedOrder.paymentDetails && selectedOrder.paymentDetails.notes && (
								<div className="bg-blue-50 p-4 rounded-lg">
									<h3 className="font-medium text-gray-900 mb-2">Payment Notes</h3>
									<p className="text-sm text-gray-600">{selectedOrder.paymentDetails.notes}</p>
								</div>
							)}
							...
							{/* Sales Rep Info */}
							{selectedOrder.paymentDetails && selectedOrder.paymentDetails.processedBy === "sales_rep" && (
								<div className="bg-green-50 p-4 rounded-lg">
									<h3 className="font-medium text-gray-900 mb-2">Sales Information</h3>
									<p className="text-sm text-gray-600">
										Order created by sales representative on{" "}
										{selectedOrder.paymentDetails.processedAt ? 
											formatDate(selectedOrder.paymentDetails.processedAt) : 
